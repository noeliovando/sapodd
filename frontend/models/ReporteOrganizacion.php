 ...
 */
class ReporteOrganizacion extends \yii\db\ActiveRecord
{
    /**
     * @inheritdoc
     */
    ...
    public function getHorasOrganizaciones($id_organizacion)
    {
        $anio= date('Y');
        $mes = date('M');
        $desde = ($anio-1).'-'.'12'.'-26';
        ...
        $count = Reporte::find()
            ...
            ->andwhere(['id_proyecto'=> $id_organizacion])
            ->joinWith('idAnalista')
            ->andWhere([
    ...
    {
        return ArrayHelper::map(Organizacion::find()->asArray()->all(), 'id', 'nombre');
    }
